...
torch::Tensor non_max_supperession(torch::Tensor& prediction,
...
TorchInference::TorchInference(fs::path model_path, fs::path class_file)
    : IInference(model_path, class_file),
      device_(torch::cuda::is_available() ? torch::kCUDA : torch::kCPU) {
  // populate classes here
  classes_ = {"snake", "background"};
...
std::tuple<cv::Mat, std::string, float> TorchInference::Infer(
    cv::Mat in_image) {
  // Load image and preprocess
  cv::Mat input_image;
  letterbox(in_image, input_image, {384, 384});
